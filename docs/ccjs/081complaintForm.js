...
solutionRadios.forEach((radio) => {
  radio.addEventListener("change", () => {
    toggleField(otherSolutionTextBox, otherSolutionRadio.checked);
  });
});
...
function changeBorder(input) {
  input.checkValidity()
    ? (input.style.borderColor = "var( --accent-color-dteal)")
    : (input.style.borderColor = "var( --accent-color-gold)");
}
...
function changeGroupBorder(buttonType, buttonGroup) {
  buttonsClicked(buttonType)
    ? (buttonGroup.style.borderColor = "var( --accent-color-dteal)")
    : (buttonGroup.style.borderColor = "var( --accent-color-gold)");
}
...
function validateForm() {
  ...
  buttonsValidation(complaintBoxes, complaintsGroup, otherComplaintText, otherComplaintBox.checked);

  buttonsValidation(solutionRadios, solutionsGroup, otherSolutionText, otherSolutionRadio.checked);
