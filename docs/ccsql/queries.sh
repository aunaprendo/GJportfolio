...
echo "$($PSQL "
SELECT winner.name AS name
FROM games
JOIN teams AS winner
ON games.winner_id = winner.team_id
WHERE round = 'Eighth-Final' AND year = 2014

UNION

SELECT opponent.name AS name
FROM games
...
echo "$($PSQL "SELECT year, winner.name FROM games JOIN teams AS winner ON games.winner_id = winner.team_id WHERE round = 'Final' ORDER BY year")"
